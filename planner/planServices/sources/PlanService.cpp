...
#include "../../sqlService/headers/SQLConnection.h"
...
void PlanService::createDailyPlan( const date &d, const unique_ptr < View > &view )
{
    ...
    unique_ptr < SQLConnection > sqlConnection( new SQLConnection() );
    sqlConnection->addPlan( move( plan ) );
}
...
void PlanService::createMonthlyPlan( const string &month, const unique_ptr < View > &view )
{
    string description = view->getDescriptionFromUser();
    string category = view->getCategoryFromUser();
    unique_ptr < MonthlyPlan > plan( new MonthlyPlan( description, category, month ) );
...
void PlanService::markDone( const shared_ptr < Plan > &plan )
{
    ...
    unique_ptr < SQLConnection > sqlConnection( new SQLConnection() );
    sqlConnection->markDone( plan );
}

void PlanService::removePlan( const shared_ptr < Plan > &plan )
{
    unique_ptr < SQLConnection > sqlConnection( new SQLConnection() );
    sqlConnection->deletePlan( plan );
}

string PlanService::getMonthFromUserForCreatingPlan( const unique_ptr < View > &view )
{
    array < string, 12 > monthsArray {"January", "February", "March", "April", "May", "June", "July", "August",
                                      "September", "November", "October", "December"};
    string month;
    array < string, 12 >::iterator monthIterator;
    int currentMonth = day_clock::local_day().month().as_number();
    int monthNumber = 0;
    ...
    do
    {
        ...
        //if user didn't enter a month - set it to the next one
        if ( month.empty() )
        {
            months addedMonth( 1 );
            month = ( day_clock::local_day() + addedMonth ).month().as_long_string();
            return month;
        }
            //check whether a month entered by user is valid (whether it exists and is current or later one)
        else if ( ( monthIterator = find( monthsArray.begin(), monthsArray.end(), month ) ) != monthsArray.end() )
        {
            monthNumber = monthIterator - monthsArray.begin() + 1;
        }
